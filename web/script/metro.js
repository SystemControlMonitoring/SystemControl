/*
 *  metro.js - Windows 8 Metro UI [v1.2]
				...
				if( a.clicking==true ){
					...
					if( el.hasClass('sidebar') && !el.data('drag').happening ){
						if( allEl.index(el) == $('#metroSidebar').data('whoOpenedIt') )
							$('#metroSidebar').animate({'width':'hide'},'fast').empty().removeData('whoOpenedIt')
						else 
							$('#metroSidebar').load(el.attr('href')).animate({'width':'show'},'fast').data('whoOpenedIt',allEl.index(el))
					}
